...
A sparse tensor BSR format is a generalization of sparse tensor CSR
format where BSR indices refer to 2-dimensional dense blocks in the
tensor rather than elements as in the case of a CSR tensor. For
example:
```python
...
3.8 `crow_indices.size() == (nrows + 1,)`

3.9 `col_indices.size() == (nnz,)`

3.10 `values.size() == (nnz, blocksize[0], blocksize[1])`
...
3.8 `crow_indices.size() == batchsize + (nrows + 1,)`

3.9 `col_indices.size() == batchsize + (nnz,)`

3.10 `values.size() == batchsize + (nnz,) + blocksize + densesize`
...
5.6 `col_indices[..., crow_indinces[..., i-1]:crow_indinces[..., i]]` must be sorted and with distinct values for all `i=1,...,nrows`

where ellipses `...` reads `i1, ..., iM` and invariants apply for all `0 <= i1 < batchsize[0], ..., 0 <= iM < batchsize[M-1]`.
